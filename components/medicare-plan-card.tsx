...
function buildBenefitCells(plan: MedicarePlan): BenefitCell[] {
  ...
  const otcAmt = plan.otcAllowanceAmount ?? parseDollar(b.otcAllowance);
  ...
  return [
    ...
    {
      // 4th cell prefers OTC ($) when the plan has an OTC allowance, else Rx.
      name: otcAmt && otcAmt > 0 ? "OTC" : "Rx Drugs",
      bold: otcAmt && otcAmt > 0 ? fmtDollar(otcAmt) : (rxRaw || "—"),
      value: otcAmt && otcAmt > 0 ? "/mo allowance" : (rxRaw ? "" : ""),
      icon: otcAmt && otcAmt > 0 ? (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      tip: otcAmt && otcAmt > 0 ? `${fmtDollar(otcAmt)}/mo OTC allowance` : "Tier 1 prescription drug coverage",
    },
  ];
}
...
export default function MedicarePlanCard({
  ...
  const givebackAmt = partBGivebackAmount ?? parseDollar(benefits.partBGiveback) ?? 0;
  const benefitCells = buildBenefitCells(plan);
  const logoSrc = carrierLogo(carrier);
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className={`w-8 h-8 rounded-full grid place-items-center border transition-all ${
            saved ? "text-[#a43499] border-[#a43499]/35 bg-[#a43499]/[0.06]" : "text-[#a097aa] border-[#ece8f1] hover:text-[#a43499] hover:border-[#a43499]/35"
          }`}
      ...
      {/* Plan name */}
      <h3 className="text-center font-semibold text-[17px] text-[#1f1330] leading-[1.25] min-h-[44px] flex flex-col justify-center px-2">
        {name}
      </h3>

      {/* Meta strip: plan ID + optional pink SNP badge */}
      <div className="flex items-center justify-center gap-2 mt-1 text-[11px] text-[#7e8196]">
        <code className="font-mono">{shortPlanId(id)}</code>
        {snp && (
          <>
            <span className="w-[3px] h-[3px] rounded-full bg-[#d3cddc]" aria-hidden="true" />
            <span
              className="text-white bg-[#a43499] px-[7px] py-[2px] rounded text-[10px] font-bold uppercase tracking-[0.04em]"
              title={
      ...
      <StarRating rating={starRatingOverall} />

      {/* Faded divider between rating and premium */}
      ...
      {givebackAmt > 0 && <GivebackPill amount={givebackAmt} />}

      {/* Drug estimate (when meds are entered) */}
      {drugEstimate && drugEstimate.annualCost > 0 && (
        <div className="mt-2.5 mx-auto flex items-center justify-center gap-1.5 bg-[#f4eef9] text-[#6a2fa0] border border-[#e2d3f0] rounded-lg px-3 py-2 text-[12.5px] font-medium w-fit max-w-full text-center leading-snug">
          <b className="font-bold">{fmtDollar(drugEstimate.annualCost)}</b>
          &nbsp;est. annual Rx cost
          {drugEstimate.uncoveredDrugs.length > 0 && (
            <span className="text-[11px] text-[#6a6378]"> · {drugEstimate.uncoveredDrugs.length} not covered</span>
          )}
        </div>
      )}

      {/* Stats grid 2x2 — Deductible/MOOP, then Primary/Specialist */}
      <div className="grid grid-cols-2 border-t border-b border-[#ece8f1] mt-3.5 text-[13px]">
        <div className="px-1.5 py-2.5 text-center flex flex-col gap-0.5">
        ...
        <div className="px-1.5 py-2.5 text-center flex flex-col gap-0.5 border-l border-[#ece8f1]">
        ...
        <div className="px-1.5 py-2.5 text-center flex flex-col gap-0.5 border-t border-[#f3f0f7]">
          <span className="text-[#6a6378] text-[12px]">Primary Copay</span>
          <span className="font-semibold text-[14px] text-[#1f1330]">{benefits.primaryCare || "—"}</span>
        </div>
        <div className="px-1.5 py-2.5 text-center flex flex-col gap-0.5 border-l border-[#ece8f1] border-t border-t-[#f3f0f7]">
          <span className="text-[#6a6378] text-[12px]">Specialist</span>
          <span className="font-semibold text-[14px] text-[#1f1330]">{benefits.specialist || "—"}</span>
        </div>
      </div>
